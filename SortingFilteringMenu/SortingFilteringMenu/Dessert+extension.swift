...
extension Dessert {
    ...
    class func consumed(by customer: Customer, _ context: NSManagedObjectContext) -> [Dessert] {
        let request = Dessert.request()
        let predicate = NSPredicate(format: "(ANY fromCustomer == %@", customer)
        request.predicate = predicate

        do {
            guard let results = try context.fetch(request) as? [Dessert] else { return [] }
            return results
        } catch let error {
    ...
    static func consumedByJohnDoe(_ context: NSManagedObjectContext) -> [Dessert] {
        guard let customer = Customer.with(fullName: "John Doe", context) else { return [] }
        return consumed(by: customer, context)
    }
